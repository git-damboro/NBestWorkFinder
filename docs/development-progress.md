...
| 优先级 | 模块 | 任务 | 说明 |
|---:|---|---|---|
| P0 | `resume / interview / jobdraft` | 回归 `BUG-008` ~ `BUG-010` | 代码修复已完成，待页面手测确认 |
| P1 | `knowledgebase / rag-chat` | 回归 `BUG-011` ~ `BUG-013` | 代码修复已完成，待页面手测确认 |
...
|---|---|
| 目标 | 完成两批手测问题回归 |
| 具体任务 | 第一批 `BUG-008` ~ `BUG-010`、第二批 `BUG-011` ~ `BUG-013` 均已完成代码修复，下一步集中页面回归 |
...
| `待提交` | fix | 修复简历重新分析入口、开始面试空页兜底提示，并记录职位草稿编辑约束根因 |
| `待提交` | fix | 修复问答助手会话列表懒加载风险、新建对话行为和重向量化入口展示 |
