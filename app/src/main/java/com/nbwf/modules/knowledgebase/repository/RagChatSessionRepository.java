...
import com.nbwf.modules.knowledgebase.model.RagChatSessionEntity.SessionStatus;
...
public interface RagChatSessionRepository extends JpaRepository<RagChatSessionEntity, Long> {
     ...
     */
    @Query("SELECT DISTINCT s FROM RagChatSessionEntity s LEFT JOIN FETCH s.knowledgeBases WHERE s.userId = :userId ORDER BY s.isPinned DESC, s.updatedAt DESC")
